...
def query_account_qa(user_query: str, top_k: int = 5):
    ...
    if USE_FLAN_CLEANING:
        ...
        prompt = f"get the answer in the key:value pairs to plot the graph: {top_result}"
        rephrased_kv_pairs = flan_pipeline(prompt, max_length=312, do_sample=False)[0]["generated_text"]
        results[0]["kv_pairs"] = rephrased.strip()
    print(top_result)
    return {
